/* 
    ...
    In this file The disease list data structure is implemented. In this list
    each individual virus is stored once. Each node has a skip list pointer to 
    a vaccinated skip list and a non vaccinated skip list. Also a bloomfilter type
    pointer and a stats_head pointer to a stats type node which points to 2 
    ordinary lists of accepted and rejected requests.
    This list is used both by the coordinator and monitor proccesses. The coordinator
    doesn't use the skip list pointers and the monitor the stats pointer.
  */
...
struct Disease_List *Disease_List_Init(char *virusName) {
  ...
  newNode->bf = NULL;
  newNode->stats_head =NULL;//for travelStats assistive structure
...
void Disease_List_Insert(struct Disease_List **head, char *virusName, struct Hash_Entry *hptr, int bloomSize) {
  ...
  temp->bf = BloomFilter_Creation(bloomSize);
}
...
void Disease_List_Purge(struct Disease_List *head) {
  ...
  while (temp != NULL) {
    ...
    BloomFilter_Purge(thr->bf);
    StatsHead_Purge(thr->stats_head);
