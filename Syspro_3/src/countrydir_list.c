...
struct CountryDirList *CountryDirList_Init(char *country,int monitor_id) {
...
void CountryDirList_Insert(struct CountryDirList **head, char *country, int monitor_id) {

    struct CountryDirList *newNode = CountryDirList_Init(country,monitor_id);
    char cur, new;
    if (*head != NULL) {
        cur = (*head)->country[0];
        new = country[0];
...
void CountryDirList_Print(struct CountryDirList *head) {
    ...
    while (temp != NULL) {
        if (temp->input && temp->output) {
            printf("%20s\t fd:%d\t pid:%d \n", temp->country, temp->input->fd, temp->pid);
        } else {
            printf("%20s %5d %5d %3d \n", temp->country, 0, 0, 0);
        }
        temp = temp->next;
